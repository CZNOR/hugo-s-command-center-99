...
const statusConfig = {
  active: { label: "Actif", color: "bg-hugoos-green/10 text-hugoos-green", dot: "bg-hugoos-green" },
  paused: { label: "En pause", color: "bg-hugoos-orange/10 text-hugoos-orange", dot: "bg-hugoos-orange" },
          ...
          return (
            <motion.div key={agent.id} variants={fadeUp} className="glass-card-hover p-5 cursor-pointer group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold" style={{ background: "hsl(189 94% 43% / 0.1)", color: "hsl(189 94% 33%)" }}>
                      {agent.avatar}
                    </div>
                    <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white ${s.dot}`} />
                  </div>
                  <div>
              ...
              <div className="flex items-center gap-1.5 mb-3">
                ...
                {agent.tools.map((t) => (
                  <span key={t} className="text-[10px] px-1.5 py-0.5 rounded-lg bg-black/[0.04] text-muted-foreground">{t}</span>
                ))}
              </div>
      ...
      <motion.div variants={fadeUp} className="glass-card p-5">
        ...
        <div className="space-y-2">
          {agentLogs.map((log, i) => {
            ...
            return (
              <div key={i} className="flex items-start gap-3 p-2.5 rounded-xl hover:bg-black/[0.03] transition-colors">
